...
onready var ammo_source = preload ("res://ammo.tscn")
#allows the script to use the player
onready var player = $"../../Player"
#loads the spawn point of the box, currently not functioning
onready var ammo_spawn = get_node("ammoSpawn")

#moves the enemy towards the player, the "if direction.length() is responisble for how far away the enemy should stay
...
#if the player enters the enemies body, takes away ten from the player health value in the global script
...
func take_damage(damage):
	...
	if enemy_health <=0:
		...
		var ammo = ammo_source.instance()
		get_node("/root/devil_hunter").add_child(ammo)
